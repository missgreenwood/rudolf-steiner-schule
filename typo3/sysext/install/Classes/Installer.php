...
class Installer {
	...
	public function __construct() {
		...
		if (!$GLOBALS['TYPO3_CONF_VARS']['BE']['installToolPassword'] && !defined('APS_UPGRADE')) {
		###	END PATCH AUTO-UPGRADE	###
		...
		if ($this->sendNoCacheHeaders && !defined('APS_UPGRADE')) {
		###	END PATCH AUTO-UPGRADE	###
		...
		if (defined('APS_UPGRADE') || $this->session->isAuthorized() || $this->checkPassword()) {
		###	END PATCH AUTO-UPGRADE	###
			$this->passwordOK = 1;
			...
			if($this->redirect_url && !defined('APS_UPGRADE')) {
			###	END PATCH AUTO-UPGRADE	###
			...
			!defined('APS_UPGRADE') && $this->formProtection->injectInstallTool($this);
			###	END PATCH AUTO-UPGRADE	###
		} else {
			$this->loginForm();
		}
...
REMOTE_ADDR was \'' . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('REMOTE_ADDR') . '\' (' . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('REMOTE_HOST') . ')';
	...
	public function init() {
		...
		if (!defined('APS_UPGRADE') && count($this->messages)) {
		###	END PATCH AUTO-UPGRADE	###
	...
	public function checkTheDatabase() {
		...
		if ($action_type) {
			...
			case 'cmpFile':
				...
				if ($tblFileContent) {
					...
					if (defined('APS_UPGRADE') || is_array($this->INSTALL['database_update'])) {
					###	END PATCH AUTO-UPGRADE	###
		...
		###	PATCH AUTO-UPGRADE	###
		//$this->output($this->outputWrapper($this->printAll()));
		defined('APS_UPGRADE') || $this->output($this->outputWrapper($this->printAll()));
		###	END PATCH AUTO-UPGRADE	###
	}
	...
	public function updateWizard_parts($action) {
		...
		case 'performUpdate':
			...
			foreach ($this->INSTALL['update']['extList'] as $identifier) {
				...
				if (!defined("APS_UPGRADE") && method_exists($tmpObj, 'checkUserInput') && !$tmpObj->checkUserInput($customOutput)) {
					$customOutput = '';
					...
					if (defined("APS_UPGRADE") && method_exists($tmpObj, 'checkForUpdate')) {
						$shouldUpdate = 0;
						$explanation = '';
						if (!$tmpObj->checkForUpdate($explanation, $shouldUpdate)) {
							continue;
						}
					}
